...
export async function login(data: LoginRequest) {
  ...
  const usuario = await prisma.user.findUnique({ where: { mail: data.email } });
  if (!usuario) {
  ...
  // Comparar contraseña
  const ok = await bcrypt.compare(data.password, usuario.contrasena);
  if (!ok) {
    const e: any = new Error('Credenciales inválidas');
    e.statusCode = 401;
    throw e;
  ...
  const payload = { id: usuario.id, email: usuario.mail};
